...
# Step 2 (in progress) — extract L2 Residual corrections
...
g++ -std=c++17 $(root-config --cflags) tests/TestDijet.cxx -o /tmp/TestDijet $(root-config --libs) -I include
/tmp/TestDijet
```
...
### Step 2 — Residuals Extraction *(in progress)*

For each (alpha threshold, pT_avg slice, |η| bin):

1. Apply `SetRangeUser` on the alpha and pT_avg axes of the THnSparse
2. Project to 1D asymmetry distribution
3. Extract mean asymmetry ⟨A⟩
4. Compute response R = (1 + ⟨A⟩) / (1 − ⟨A⟩)
5. Form the DATA/MC double ratio
6. Fit the alpha dependence linearly and extrapolate to alpha = 0

The intercept is the L2 Residual correction for that (pT_avg, |η|) bin.

### Step 3 — Text File Output *(in progress)*
...
macros/
  runAsymmetry.C     Step 1 entry point — unified DATA/MC, configured by --mc flag
  runResiduals.C     Step 2 entry point — in progress

src/
  RunAsymmetry.cxx       Step 1 implementation
  ResidualsExtractor.cxx Step 2 implementation — in progress

tests/
  TestDijet.cxx          37 tests — FindLeadingJets and MakeDijet
  TestFoldEtaAxis.cxx     7 tests — FoldEtaAxis

readme/              versioned READMEs (this is v4)
```
...
TH1D* proj = (TH1D*)h->Projection(kEtaAxis);
```
...
The THnSparse stores full η_probe across all 36 CMS JEC bins. At extraction, `FoldEtaAxis()` in `Utilities.h` produces a new THnSparse with |η| as the axis. This lets you inspect individual η bins (and check forward-backward symmetry) before committing to the fold, without storing duplicate data at fill time.
...
Alpha fits for the extrapolation use thresholds up to 0.30; above this, trigger bias distorts the distribution.

---

## Adapting for a New Run Period

Add a new config header under `configs/` specifying:

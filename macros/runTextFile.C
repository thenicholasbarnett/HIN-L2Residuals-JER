// Compiled:    ./bin/runTextFile <hp_residuals.root> <zb_residuals.root> <output.root> <output_text_prefix> [method] [norm]
...
// norm:   direct (default) | norm  — use the kFSR-normalized intercepts instead
...
int main( int argc, char* argv[] ){
    if( argc < 5 ){
        std::cerr << "Usage: runTextFile <hp_residuals.root> <zb_residuals.root> <output.root> <output_text_prefix> [method] [norm]\n";
        return 1;
    }
    TString method = ( argc > 5 ) ? argv[5] : "gauss";
    bool useNorm = ( argc > 6 ) && TString( argv[6] ) == "norm";

...
static bool HasH( TFile* f, const TString& name ){
    return f && f->Get( name );
}

static bool HasH( TDirectory* d, const TString& name ){
    return d && d->Get( name );
}

static bool HasHAny( TFile* f, const std::vector<TString>& names ){
    for( const auto& name : names ) if( HasH( f, name ) ) return true;
    return false;
}

static bool HasGraphWithN( TDirectory* d, const std::vector<TString>& names, int minN ){
    if( !d ) return false;
    for( const auto& name : names ){
...
static int CountAsymDistPlots( TFile* fIn, const TString& cone, const BinningConfig& bins ){
    TDirectory* dData = ( TDirectory* )fIn->Get( cone + "/QA_data" );
    TDirectory* dMC   = ( TDirectory* )fIn->Get( cone + "/QA_mc" );
...
static int CountROverlayPlots( TFile* fIn, const TString& cone, const BinningConfig& bins ){
    TDirectory* dRvals = ( TDirectory* )fIn->Get( cone + "/Rvals" );
    if( !dRvals ) return 0;
...
static int CountAlphaFitPlots( TFile* fIn, const TString& cone, const BinningConfig& bins ){
    TDirectory* dGraphs = ( TDirectory* )fIn->Get( cone + "/graphs" );
    if( !dGraphs ) return 0;
...
static int CountPtFitPlots( TFile* fIn, const TString& cone ){
    TDirectory* dGraphs = ( TDirectory* )fIn->Get( cone + "/graphs" );
    if( !dGraphs ) return 0;
...
static int CountEtaSymPlots( TFile* fIn, const TString& cone, const BinningConfig& bins ){
    int n = 0;
    for( int m = 0; m < kNMethods; m++ ){
...
static int CountMethodCompPlots( TFile* fIn, const TString& cone, const BinningConfig& bins, bool fullEta ){
    int n = 0;
    const TString etaMode = L2Name::EtaModeKey( fullEta );
    for( const auto& sl : bins.ptavgSlices ){
...
static int CountFinalsPlots( TFile* fIn, const TString& cone, const BinningConfig& bins ){
    int n = 0;
    for( int m = 0; m < kNMethods; m++ ){
        for( int mode = 0; mode < 2; mode++ ){
            const bool fullEta = ( mode == 1 );
...
static int CountNormCompPlots( TFile* fIn, const TString& cone, const BinningConfig& bins, bool fullEta ){
    int n = 0;
    const TString etaMode = L2Name::EtaModeKey( fullEta );
    for( int m = 0; m < kNMethods; m++ ){
        bool anyValid = false;
...
static int CountKinematicsPlots( TFile* fIn, const TString& cone ){
...
static int CountEventPlots( TFile* fIn ){
    int n = 0;

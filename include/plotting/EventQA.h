...
inline void PlotEvent(TFile *fIn, const TString &outDir, ProgressBar &pb) {
  ...
  // ---- vz: all events vs after cuts ----
  {
    ...
    if (hall && hvz) {
      TH1D *hallc = (TH1D *)hall->Clone("hvz_all_c");
      hallc->SetDirectory(0);
      TH1D *hvzc = (TH1D *)hvz->Clone("hvz_c");
      hvzc->SetDirectory(0);
      ...
      leg->AddEntry(
          hallc,
          Form("All events (%s)",
               FormatEntriesText((Long64_t)hall->GetEntries()).Data()),
          "f");
      leg->AddEntry(hvzc,
                    Form("After cuts  (%s)",
                         FormatEntriesText((Long64_t)hvz->GetEntries()).Data()),
                    "lp");
      leg->Draw();
      CMS_lumi(c, 16, 11);

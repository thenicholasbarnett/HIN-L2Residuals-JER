...
static const KinematicsCollection kKinematicsCollections[] = {
    {"incl", "inclusive"}, {"probe", "probe"}, {"tag", "tag"}};
...
inline TString PtMinKey(double ptMin) { return Form("ptmin_%g", ptMin); }
...
inline void PlotKinematics(TFile *fIn, const TString &outDir,
  ...
  for (int ic = 0; ic < kNKinematicsCollections; ic++) {
    ...
    {
      TString cvName =
          Form("kinematics_%s_%s_pt", cone.Data(), collPlot.Data());
    ...
    {
      TString cvName =
          Form("kinematics_%s_%s_eta", cone.Data(), collPlot.Data());
      ...
      hEtaAll[ic] = (TH1D *)h->Clone(Form("hEtaAll_%d", ic));
      hEtaAll[ic]->SetDirectory(0);
      DrawCMSInternalHeader(0.14, 0.90);
      drawConeLabel(collPlot, 0.14);
      SaveKinematicsPlot(c, outDir, cone, collPlot, cvName);
      delete c;
    ...
    {
      TString cvName =
          Form("kinematics_%s_%s_phi", cone.Data(), collPlot.Data());
      ...
      hPhiAll[ic] = (TH1D *)h->Clone(Form("hPhiAll_%d", ic));
      hPhiAll[ic]->SetDirectory(0);
      DrawCMSInternalHeader(0.14, 0.90);
      drawConeLabel(collPlot, 0.14);
      SaveKinematicsPlot(c, outDir, cone, collPlot, cvName);
      delete c;
    ...
    for (int ip = 0; ip < kNKinematicsPtMins; ip++) {
      ...
      if (!collTitle.IsNull())
        collTitle[0] = toupper(collTitle[0]);
      TLatex *tColl =
          new TLatex(0.48, 0.895, Form("%s jets", collTitle.Data()));
      tColl->SetNDC();
      tColl->SetTextFont(42);
      tColl->SetTextSize(0.038);
      tColl->SetTextAlign(22);
      tColl->Draw();
      ...
      tPt->SetNDC();
      tPt->SetTextFont(42);
      tPt->SetTextSize(0.035);
      tPt->SetTextAlign(11);
      tPt->Draw();

      TLatex *tCone = new TLatex(0.84, 0.045, cone.Data());
      tCone->SetNDC();
      tCone->SetTextFont(42);
      tCone->SetTextSize(0.035);
      tCone->SetTextAlign(31);
      tCone->Draw();
  ...
  auto DrawOverview1D = [&](TH1D *hArr[], const TString &xTitle,
    ...
    // normalize
    for (int ic = 0; ic < kNKinematicsCollections; ic++) {
      if (hArr[ic] && hArr[ic]->Integral() > 0)
        hArr[ic]->Scale(1.0 / hArr[ic]->Integral());
    }

    Color_t cols[kNKinematicsCollections] = {HiroshigeNightBlue(),
                                             HiroshigeBlue(), KlimtRed()};
    int markers[kNKinematicsCollections] = {20, 21, 22};
    for (int ic = 0; ic < kNKinematicsCollections; ic++) {
      if (hArr[ic])
        StyleH(hArr[ic], cols[ic], markers[ic]);
    }
    ...
    for (int ic = 0; ic < kNKinematicsCollections; ic++)
      if (hArr[ic])
        ymax = std::max(ymax, hArr[ic]->GetMaximum());
    ...
    if (logy)
      hFrame->SetMinimum(1e-6);

    TString cvName =
        Form("kinematics_%s_overview_%s", cone.Data(), varName.Data());
    TCanvas *c = new TCanvas(cvName, "", 800, 800);
    RealAspectRatio(c);
    c->SetLeftMargin(0.14);
    c->SetGridx();

    ...
    for( int m = 0; m < kNMethods; m++ ){
        for( int ip = 0; ip < nPt; ip++ ){
            ...
            for( int ie = 0; ie < nEta; ie++ ){
                ...
                TString gname = L2Name::ObjectName( cone, CalibKind( "R", useJer ),
            ...
            RealAspectRatio( c );
                ...
                tex->DrawLatex( 0.14, 0.92, Form( "%s  |  %s  |  %s  |  |#eta| bin %d  |  %s",
                    cone.Data(), kMethodLabels[m], ptSl.title.Data(), ie, CalibTag( useJer ).Data() ) );

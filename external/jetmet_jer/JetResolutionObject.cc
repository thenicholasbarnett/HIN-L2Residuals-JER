...
namespace JME {
  ...
  std::vector<float> JetParameters::createVector(const std::vector<std::string>& binname) const {
    ...
    for (const auto& name : binname) {
      ...
      if (it == m_values.cend()) {
        // Upstream calls edm::LogPrint("JPM") here unconditionally, but the
        // STANDALONE shim namespace above only defines edm::errors::ErrorCode,
        // not edm::LogPrint -- a genuine compile-time bug in this file's
        // STANDALONE support (this is the exact codepath getRecord() uses via
        // createVector(vector<string>), so it can't be avoided by not calling
        // it). Patched locally, guarded the same way every other STANDALONE
        // branch in this file already is.
#ifndef STANDALONE

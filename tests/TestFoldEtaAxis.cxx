...
THnSparse* MakeTestSparse( const char* name ){
    ...
    THnSparse* h = MakeTHnSparse<THnSparseD>( name, "test", { bins.eta, asymBins, ptBins } );
    SetEtaBins( h, 0 );
    h->Sumw2();
    return h;
}
...
int main(){
    ...
    std::cout << "\n[1] Input axis properties" << std::endl;
    {
        ...
        Check( std::abs( h->GetAxis( 0 )->GetXmin() - ( -5.191 )) < kEps, "eta axis lo = -5.191" );
        Check( std::abs( h->GetAxis( 0 )->GetXmax() - 5.191 ) < kEps, "eta axis hi =  5.191" );
        Check( std::abs( h->GetAxis( 0 )->GetBinLowEdge( 1 ) - kEtaEdges[0] ) < kEps, "bin 1 low edge matches kEtaEdges[0]" );
        Check( std::abs( h->GetAxis( 0 )->GetBinUpEdge( 36 ) - kEtaEdges[36] ) < kEps, "bin 36 up edge matches kEtaEdges[36]" );
        delete h;
    }
    ...
    std::cout << "\n[2] Folded axis properties" << std::endl;
    {
        ...
        Check( std::abs( hf->GetAxis( 0 )->GetXmin() - 0.0 ) < kEps, "|eta| axis lo = 0" );
        Check( std::abs( hf->GetAxis( 0 )->GetXmax() - 5.191 ) < kEps, "|eta| axis hi = 5.191" );
        Check( std::abs( hf->GetAxis( 0 )->GetBinLowEdge( 1 ) - kAbsEtaEdges[0] ) < kEps, "folded bin 1 low edge matches kAbsEtaEdges[0]" );
        Check( std::abs( hf->GetAxis( 0 )->GetBinUpEdge( 1 ) - kAbsEtaEdges[1] ) < kEps, "folded bin 1 up edge = 0.261" );
        Check( std::abs( hf->GetAxis( 0 )->GetBinUpEdge( 18 ) - kAbsEtaEdges[18] ) < kEps, "folded bin 18 up edge = 5.191" );
        ...
        Check( std::abs( hf->GetAxis( 1 )->GetXmin() - ( -1.0 )) < kEps, "asymmetry axis lo unchanged" );
        Check( std::abs( hf->GetAxis( 2 )->GetXmax() - 1000.0 ) < kEps, "pT_avg axis hi unchanged" );
    ...
    std::cout << "\n[3] Symmetric fill" << std::endl;
    {
        ...
        Check( std::abs( err - TMath::Sqrt( 2.0 )) < kEps, "symmetric fill: error = sqrt(2)" );
    ...
    std::cout << "\n[4] Positive-only fill" << std::endl;
    {
        ...
        Check( std::abs( hf->GetBinContent( c ) - 3.0 ) < kEps, "positive fill: content = 3.0" );
        Check( std::abs( hf->GetBinError( c ) - 3.0 ) < kEps, "positive fill: error = 3.0" );
    ...
    // ----------------------------------------------------------------
    std::cout << "\n[5] Negative-only fill maps to correct |eta| bin" << std::endl;
    {
        ...
        Check( std::abs( hf->GetBinContent( c ) - 5.0 ) < kEps, "negative fill: content = 5.0" );
        Check( std::abs( hf->GetBinError( c ) - 5.0 ) < kEps, "negative fill: error = 5.0" );

        // also confirm no content anywhere else on the |eta| axis
        bool onlyOneBinFilled = true;
        for( Int_t ib = 1; ib <= 18; ib++ ){
            if( ib == absEtaBin ) continue;
            Int_t cc[3] = { ib, aBin, ptBin };
            if( hf->GetBinContent( cc ) != 0.0 ){ onlyOneBinFilled = false; break; }
        }
        Check( onlyOneBinFilled, "negative fill: only the correct |eta| bin is filled" );
    ...
    std::cout << "\n[7] FoldEtaAxis works on arbitrary axis index" << std::endl;
    {
        ...
        THnSparse* h = MakeTHnSparse<THnSparseD>( "h7", "test", { asymBins, bins.eta, ptBins } );
        SetEtaBins( h, 1 );
        h->Sumw2();
        ...
        Check( hf->GetAxis( 1 )->GetNbins() == 18, "folding axis 1: result has 18 |eta| bins" );
        Check( hf->GetAxis( 0 )->GetNbins() == 10, "folding axis 1: axis 0 (asymmetry) unchanged" );
        Check( hf->GetAxis( 2 )->GetNbins() == 5, "folding axis 1: axis 2 (pT_avg) unchanged" );
        ...
        Check( std::abs( hf->GetBinContent( c ) - 2.0 ) < kEps, "arbitrary axis fold: content = 2.0" );
        Check( std::abs( hf->GetBinError( c ) - TMath::Sqrt( 2.0 )) < kEps, "arbitrary axis fold: error = sqrt(2)" );

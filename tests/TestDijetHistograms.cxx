...
int main(){
    ...
    std::cout << "\n[3] FillInclJetResp: matched jet" << std::endl;
    {
        ...
        Check( std::fabs( content - 1.0 ) < kEps, "matched jet fills all three response ratios correctly" );
    }
    ...
    std::cout << "\n[5] FillResp: tag matched, probe unmatched" << std::endl;
    {
        ...
        Double_t x[5] = { eta[0], refPt[0], pt[0] / refPt[0], jtPt[0] / refPt[0], rawPt[0] / refPt[0] };
        Int_t coords[5];
        for( Int_t i = 0; i < 5; i++ ) coords[i] = h.hTagJetResp->GetAxis( i )->FindBin( x[i] );
        Double_t content = h.hTagJetResp->GetBinContent( coords );
        Check( std::fabs( content - 1.0 ) < kEps, "tag response ratios = corrPt/refPt=1.21875, jtPt/refPt=0.96875, rawPt/refPt=0.9375" );
    }
